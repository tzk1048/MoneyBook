...
const listshort_credit = document.getElementById('listshort_credit');
...
listshort_credit.innerHTML = document.getElementById('shorttagcredit').innerHTML;
listshort_pay.innerHTML = document.getElementById('shorttagshop').innerHTML;

const shortcutdate = document.getElementsByClassName('shortcutdate');
const shorttagcredit = document.getElementsByClassName('shortcuttagcredit');
...
for (let index = 0; index < shorttagcredit.length; index++) {
    shorttagcredit[index].innerHTML = document.getElementById('shorttagcredit').innerHTML;
...
const Insert =()=> {
    ...
    cell2.innerHTML = "<select form='input' name='FromBID["+length+"]' class='shortcuttagcredit'>"+document.getElementById('shorttagcredit').innerHTML+"</select>";
    cell3.innerHTML = "<input type='number' min='1' max='"+max_payid+"' form='input' class='shortcutcon' onchange='ShortcutShop_num("+length+")'><select name='ConID["+length+"]' class='shortcuttagshop' onchange='ShortcutShop_tag("+length+")'>"+document.getElementById('shorttagshop').innerHTML+"</select>";
...
const ListShortSet =()=> {
    ...
    if (listshort_credit.value!=0) {
        for (let index = 0; index < shorttagcredit.length; index++) {
            shorttagcredit[index].options[listshort_credit.value-200].setAttribute("selected", "selected");
        }
    } 
...
const ListShortClear =()=>{
    ...
    listshort_credit.options[0].setAttribute("selected", "selected");
    listshort_payid.value='';
    listshort_pay.options[0].setAttribute("selected", "selected");
...
const ListShortDataClear =()=>{
    for (let index = 0; index < shortcutdate.length; index++) {
        ...
        shorttagcredit[index].options[0].setAttribute("selected", "selected");
...
const InsertRecordPay =(id)=>{
    ...
    if(tablesc.cells[2].innerText==null || tablesc.cells[2].innerText==''){
        tablesc.cells[2].innerHTML="<select name='FromBID'>"+document.getElementById('shorttagcredit').innerHTML+"</select>";
    } else {
    ...
    if(tablesc.cells[3].innerText==null || tablesc.cells[3].innerText==''){
        tablesc.cells[3].innerHTML="<select name='ConID'>"+document.getElementById('shorttagshop').innerHTML+"</select>";
    } else {
        data = data + "<input type='hidden' name='ConID' form='scform' value='"+(tablesc.cells[3].id-10000)+"'>"
    }
    ...
    tablesc.cells[7].innerHTML=data+"<input type='submit' name='input_cash' value='入力' form='scform'>";
}
...
const InsertRecordPay_list =(id)=>{
    ...
    if(tablesc.cells[2].innerText!=null && tablesc.cells[2].innerText!='') {
        listshort_credit.options[tablesc.cells[2].id-200].setAttribute("selected", "selected");
    }
    if(tablesc.cells[3].innerText!=null && tablesc.cells[3].innerText!='') {
...
const updatesc_pay =(id)=>{
    if(updtf){
        ...
        udrow.cells[3].innerHTML="<select name='FromBID' id='shortset_creditid' form='updtsc'>"+document.getElementById('shorttagcredit').innerHTML+"</select>";
        document.getElementById('shortset_creditid').options[bankid-200].setAttribute("selected", "selected");
        udrow.cells[4].innerHTML="<select name='ConID' id='shortset_shop' form='updtsc'>"+document.getElementById('shorttagshop').innerHTML+"</select>";
        ...
        udrow.cells[8].innerHTML="<form method='post' action='' id='updtsc'><input type='hidden' name='id' value='" + id + "'><input type='submit' name='updtsc' value='更新' style='background-color: blue; color: white;'></form>";
        updtf=false;
    }
}
...
const updateshortcutpay =()=>{
    if(updtf){
        var table = document.getElementById('credit_shortcut_set');  //表のオブジェクトを取得
        ...
        cell4.innerHTML="<select name='FromBID' form='insertsc'>"+document.getElementById('shorttagcredit').innerHTML+"</select>";
        cell5.innerHTML="<select name='ConID' form='insertsc'>"+document.getElementById('shorttagshop').innerHTML+"</select>";
        ...
        cell9.innerHTML="<form method='post' action='' id='insertsc'><input type='submit' name='insertsc' form='insertsc' value='更新' style='background-color: blue; color: white;'></form>"
        updtf=false;
    }
}

const MarkSetPay =()=> {
    var table=document.getElementById('credit_shortcut_set');
    ...
    var markup = document.getElementById('markup_credit');
...
const InsertFromCard =(id)=> {
    ...
    if(tablesc.cells[2].innerText!=null && tablesc.cells[2].innerText!='') {
        document.getElementById('selecttag_credit').options[tablesc.cells[2].id-201].setAttribute("selected", "selected");
    }
    if(tablesc.cells[3].innerText!=null && tablesc.cells[3].innerText!='') {
