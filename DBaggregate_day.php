<?php
require_once('db.php');
class DBaggregate_day extends DB{
    ...
    public function SumPrice($table) {
        $sql = "SELECT SUM(price) FROM $table WHERE date='{$this->id}'";
    ...
    /*public function SumPriceCredit() {
        $sql = "SELECT SUM(price) FROM credit_record WHERE date='{$this->id}'";
        $res = parent::executeSQL($sql,null);
        foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){ 
            $credit = $row[0];
        }
    ...
    public function SumPriceChokin() {
        $sql = "SELECT SUM(price) FROM chokin_record INNER JOIN chokin ON chokin_record.chokin_id=chokin.chokin_id WHERE date='{$this->id}'";
        $res = parent::executeSQL($sql,null);
        foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){ 
            $chokin = $row[0];
        }
    ...
    //当月の集計を配列にする
    //変更済
    public function SelectDayAggregate(){
        $data[0]=$this->SumPrice('payment_record');
        $data[1] = $this->SumPrice('income_record');
        $data[2] = $this->SumPrice('credit_record');
        $data[3] =$this->SumPriceChokin();
        return $data;
    }

    public function DayAggregateTable(){
        $title=['支出','収入','クレジット','貯金'];
        $value=$this->SelectDayAggregate();
        $data="<table>";
        for ($i=0; $i < count($value); $i++) { 
            $data .="<tr><td>{$title[$i]}</td><td>{$value[$i]}</td></tr>";
        }
        $data.="</table>";
        return $data;
    }
/*
    ...
    public function SelectDayPayment() {
        $sql = "SELECT id,bank,payment,price,comment FROM payment_record INNER JOIN payment ON payment_record.pay_id=payment.pay_id INNER JOIN bank ON payment_record.bank_id=bank.bank_id WHERE date='{$this->id}';";
        $res = parent::executeSQL($sql,null);
        $data = "<table>";
        $data .= "<tr><th>支払い方法</th><th>項目</th><th>金額</th><th>備考</th></tr>\n";
        foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){
            $data .= "<tr id='{$row[0]}'><td>{$row[1]}</td><td>{$row[2]}</td><td>{$row[3]}</td><td>{$row[4]}</td></tr>\n";
        }
        $data .= "</table>\n";
        return $data;
    }

    public function SelectDayChokin() {
        $sql = "SELECT id,chokin,price,comment FROM chokin_record INNER JOIN chokin ON chokin_record.chokin_id=chokin.chokin_id WHERE date='{$this->id}';";
        $res = parent::executeSQL($sql,null);
        $data = "<table>";
        $data .= "<tr><th>項目</th><th>金額</th><th>備考</th></tr>\n";
        foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){
            $data .= "<tr id='{$row[0]}'><td>{$row[1]}</td><td>{$row[2]}</td><td>{$row[3]}</td></tr>\n";
        }
        $data .= "</table>\n";
        return $data;
    }
    ...
    public function SelectDayIncome() {
        $sql = "SELECT id,income,price,comment FROM income_record INNER JOIN income ON income_record.income_id=income.income_id WHERE date='{$this->id}';";
        $res = parent::executeSQL($sql,null);
        $data = "<table>";
        $data .= "<tr><th>項目</th><th>金額</th><th>備考</th></tr>\n";
        foreach($rows = $res->fetchAll(PDO::FETCH_NUM) as $row){
